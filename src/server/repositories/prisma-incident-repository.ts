...
export class PrismaIncidentRepository implements IncidentRepository {
  ...
  async timelineForActor(actor: Actor, query: TimelineQuery): Promise<TimelineItem[] | null> {
    ...
    const personActivityWhere = query.personId ? {
      OR: [{ sourceEntity: { personId: query.personId } }, { destinationEntity: { personId: query.personId } }],
    } : {};
    const [evidence, cases, relationships, communications, financialTransactions] = await Promise.all([
      ...
      prisma.communicationRecord.findMany({ where: { ...(actor.role === UserRole.Administrator ? {} : { departmentId: actor.departmentId }), ...(query.caseId ? { caseId: query.caseId } : {}), ...(query.incidentId ? { incidentId: query.incidentId } : {}), ...personActivityWhere }, select: { id: true, communicationType: true, occurredAt: true, caseId: true, incidentId: true, sourceEvidenceId: true, sourceEntity: { select: { personId: true, displayLabel: true } }, destinationEntity: { select: { personId: true, displayLabel: true } } }, orderBy: { occurredAt: "desc" }, take: 250 }),
      prisma.financialTransaction.findMany({ where: { ...(actor.role === UserRole.Administrator ? {} : { departmentId: actor.departmentId }), ...(query.caseId ? { caseId: query.caseId } : {}), ...(query.incidentId ? { incidentId: query.incidentId } : {}), ...personActivityWhere }, select: { id: true, transactionType: true, amount: true, currency: true, occurredAt: true, caseId: true, incidentId: true, sourceEvidenceId: true, sourceEntity: { select: { personId: true, displayLabel: true } }, destinationEntity: { select: { personId: true, displayLabel: true } } }, orderBy: { occurredAt: "desc" }, take: 250 }),
    ]);
    ...
    for (const item of communications) if (withinWindow(item.occurredAt, query) && acceptsType(query, "COMMUNICATION")) items.push({ id: `communication:${item.id}`, type: "COMMUNICATION", timestamp: item.occurredAt, title: `Communication: ${item.communicationType.replaceAll("_", " ")}`, description: `${item.sourceEntity.displayLabel} → ${item.destinationEntity.displayLabel}`, sourceRecordType: "COMMUNICATION", sourceRecordId: item.id, caseId: item.caseId, incidentId: item.incidentId, personIds: [item.sourceEntity.personId, item.destinationEntity.personId].filter((id): id is string => Boolean(id)), evidenceId: item.sourceEvidenceId });
    for (const item of financialTransactions) if (withinWindow(item.occurredAt, query) && acceptsType(query, "FINANCIAL")) items.push({ id: `financial:${item.id}`, type: "FINANCIAL", timestamp: item.occurredAt, title: `Financial activity: ${item.transactionType.replaceAll("_", " ")}`, description: `${item.currency} ${item.amount.toFixed(2)} · ${item.sourceEntity.displayLabel} → ${item.destinationEntity.displayLabel}`, sourceRecordType: "FINANCIAL", sourceRecordId: item.id, caseId: item.caseId, incidentId: item.incidentId, personIds: [item.sourceEntity.personId, item.destinationEntity.personId].filter((id): id is string => Boolean(id)), evidenceId: item.sourceEvidenceId });
