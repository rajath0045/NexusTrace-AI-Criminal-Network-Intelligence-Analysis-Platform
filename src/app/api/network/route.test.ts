// @vitest-environment node

import { beforeEach, describe, expect, it, vi } from "vitest";
import { UserRole } from "@/domain/model";
import { getCurrentActor } from "@/server/auth/session";
import { getNeighborhood } from "@/server/services/graph-service";
import { GET } from "./route";

vi.mock("@/server/auth/session", () => ({ getCurrentActor: vi.fn() }));
vi.mock("@/server/services/graph-service", () => ({ getNeighborhood: vi.fn() }));

const actor = { userId: "user", email: "user@nexustrace.demo", displayName: "User", role: UserRole.Investigator, departmentId: "cyber" };

describe("network graph route", () => {
  beforeEach(() => vi.mocked(getCurrentActor).mockResolvedValue(actor));

  it("requires an authenticated session", async () => {
    vi.mocked(getCurrentActor).mockResolvedValueOnce(null);
    const response = await GET(new Request("http://localhost/api/network?focus=00000000-0000-4000-8000-000000000001"));
    expect(response.status).toBe(401);
    expect(getNeighborhood).not.toHaveBeenCalled();
  });

  it("does not distinguish an unauthorized focus from a missing graph entity", async () => {
    vi.mocked(getNeighborhood).mockRejectedValueOnce({ code: "NOT_FOUND" });
    const response = await GET(new Request("http://localhost/api/network?focus=00000000-0000-4000-8000-000000000001"));
    expect(response.status).toBe(404);
    expect(await response.text()).toBe("Graph entity not found.");
  });
});
