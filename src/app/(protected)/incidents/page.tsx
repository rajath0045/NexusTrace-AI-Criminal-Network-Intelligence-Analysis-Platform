...
import { listIncidentEntityCandidates, listIncidents } from "@/server/services/incident-service";
...
export default async function IncidentsPage() {
  ...
  const [incidents, cases, layout, entities] = await Promise.all([listIncidents(actor), listCases(actor), getWorkspaceLayout(actor, "incidents"), listIncidentEntityCandidates(actor)]);
  ...
  const widgets = [
    ...
    ...(can(actor, "INCIDENT_SUBMIT") ? [{ id: "submit-incident", content: <section className="workspace-widget-panel"><header><p className="eyebrow">Investigator contribution</p><h2>Submit incident observation</h2></header><IncidentForm action={submitIncidentAction} cases={cases} people={people} entities={entities} evidence={evidence} submitLabel="Submit for department review" /></section> }] : []),
    ...(can(actor, "INCIDENT_CREATE") ? [{ id: "create-incident", content: <section className="workspace-widget-panel"><header><p className="eyebrow">Department intelligence</p><h2>Create incident</h2></header><IncidentForm action={createIncidentAction} cases={cases} people={people} entities={entities} evidence={evidence} submitLabel="Create incident" /></section> }] : []),
  ];
