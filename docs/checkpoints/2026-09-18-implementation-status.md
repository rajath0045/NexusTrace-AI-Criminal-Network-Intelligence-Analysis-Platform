...
## 2026-09-20 — Task 14 investigation reporting and provenance complete

- Added canonical `Report`, immutable `ReportVersion`, and `ReportSourceReference` persistence through an additive Prisma migration. Each generation writes a bounded, deterministic snapshot and source references in the same transaction as its audit event; regenerating creates a new version without overwriting an earlier snapshot.
- Added authenticated `/reports` and `/reports/[id]` workspaces. Authorized operators can generate a report for an authorized Case/FIR, load only their scope’s bounded report history, inspect version history, and open a print-safe browser `Print / Save PDF` view.
- Report snapshots cleanly distinguish case/evidence/incidents/relationships/timeline/communications/financial activity (authoritative context) from current investigative leads and dismissed leads. The presentation keeps the human-review warning visible and makes no finding a verified fact.
- Added provenance references for each included source with source type and identifier, Case/Incident/Evidence linkage where present, observation time, verification/review state, and safe snapshot metadata. Evidence inspection always routes through the existing authenticated retrieval endpoint; no storage keys or raw evidence content are copied into reports.
- Enforced the existing server-side Administrator cross-department, Department User departmental, and Investigator assignment scope at report list, retrieve, generation, versioning, source, audit, and direct API boundaries. Unavailable or unauthorized reports return generic not-found behavior.
- Added service, PostgreSQL integration/authorization, and Playwright coverage for input validation, deterministic snapshot/version behavior, source immutability, secure cross-department denial, printable detail rendering, and direct API denial.
- `pnpm prisma validate`, `pnpm prisma migrate status`, `pnpm typecheck`, `pnpm lint`, 130 unit/component tests, 41 PostgreSQL integration/authorization tests, 7 Playwright end-to-end tests, and `pnpm exec next build --webpack` pass. As before, the default Turbopack build is host-blocked only by the environment’s internal CSS-worker port restriction; the Webpack production build is clean.
- A pre-existing historical migration still cannot be replayed in Prisma’s fresh shadow database because it references the later `IncidentVerificationLevel` type. This milestone does not rewrite history; the new additive migration deploys and validates against the authorized local development database.

## 2026-09-20 — Task 15 production and deployment foundation implemented

- Established `pnpm build` as the canonical Webpack production build. It generates Prisma Client first and produces a Next standalone runtime; `pnpm start` runs that standalone server. Node.js `22.12.0+` and pnpm `11.19.0` are declared deployment requirements.
- Removed the unused `AUTH_SECRET` environment requirement. NexusTrace uses opaque random database-backed sessions rather than a second signing-secret mechanism. Production now validates `DATABASE_URL`, `EVIDENCE_STORAGE_ROOT`, and HTTPS `APP_ORIGIN` (outside localhost smoke testing) at startup and exits without retaining a listener on invalid configuration.
- Added nonce-backed CSP, `nosniff`, strict referrer, frame-denial, restrictive permissions policy, and HTTPS-only HSTS. The policy grants only the narrowly necessary MapLibre/OpenFreeMap network/font/image/worker sources. `unsafe-eval` is development-only for Next/React diagnostic behavior and absent from production CSP.
- Added same-origin mutation protection for API mutations, retained Next Server Action origin protection, and introduced a bounded process-local rate-limit abstraction for login, authorized search, report generation, evidence attachment, graph traversal, and deterministic investigation endpoints. It is designed for replacement by Redis or deployment-native storage before horizontal scaling.
- Added safe structured operational logging for startup, authentication outcomes, authorization denials, report failures, health/database failures, and graph service errors. Secrets, session data, storage keys, checksums, raw evidence, and credential-bearing error strings are redacted; immutable application audit records remain separate.
- Added `/api/health` liveness/readiness reporting with only safe application/database/version fields. Live standalone smoke verification confirmed database-ready health, CSP/security headers, cross-site mutation `403`, and same-origin unauthenticated `401` behavior.
- Added a non-root multi-stage Dockerfile, `.dockerignore`, Docker Compose application/PostgreSQL/evidence volumes, a dedicated one-shot migration job, container health checks, and deployment documentation covering environment, persistence, migrations, backup/restore, recovery, update, rollback, and troubleshooting. Compose configuration validates locally; Docker image/container execution is pending because this host’s Docker daemon is not running.
- Added GitHub Actions verification using Node 22.12, pnpm 11.19, PostgreSQL 16, Prisma generation/validation/deployment, seed, TypeScript, ESLint, unit, integration, Webpack build, and Playwright. The workflow faithfully exposes the pre-existing fresh-database migration ordering defect rather than masking it.
- Final local verification: Prisma schema validation/migration status and non-destructive deploy succeed against the current authorized local database (10 migrations); 141 unit/component tests, 41 PostgreSQL integration/authorization tests, and 7 Playwright E2E tests pass; TypeScript, ESLint, Webpack production build, production startup, health, and security headers pass. Fresh-database migration and Docker image/container smoke are blocked only by the pre-existing migration ordering defect and unavailable local Docker daemon, respectively.
